...
void onTick( CBlob@ this )
{
    ...
    else if ( (pressed_a1 || swordState) && !moveVars.wallsliding ) //no attacking during a slide
    {
		if (getNet().isClient())
		{
			if(knight.swordTimer == KnightVars::slash_charge_level2-0.5*getAgility(this))
			{
				Sound::Play("AnimeSword.ogg", pos, myplayer ? 1.3f : 0.7f );
			}
			else if(knight.swordTimer == KnightVars::slash_charge-0.5*getAgility(this))
			{
				Sound::Play("SwordSheath.ogg", pos, myplayer ? 1.3f : 0.7f );
			}
		}

		if(knight.swordTimer > KnightVars::slash_charge_limit-0.5*getAgility(this))
		{
			Sound::Play("/Stun", pos, 1.0f, this.getSexNum() == 0 ? 1.0f : 2.0f);
			SetKnocked(this, 15);
		}
		
		bool strong = ( knight.swordTimer > KnightVars::slash_charge_level2-0.5*getAgility(this));
        ...
        if (knight.state == KnightStates::sword_drawn && !pressed_a1 &&
			!this.isKeyJustReleased(key_action1) && delta > KnightVars::resheath_time-0.5*getAgility(this))
        {
            knight.state = KnightStates::normal;
        }
        else if (this.isKeyJustReleased(key_action1) && knight.state == KnightStates::sword_drawn )
        {
            ...
            if (delta < KnightVars::slash_charge-0.5*getAgility(this))
            {
                if (direction == -1) {
            ...
            else if (delta < KnightVars::slash_charge_level2-0.5*getAgility(this))
            {
			...
			else if (delta < KnightVars::slash_charge_limit-0.5*getAgility(this))
            {
				...
				knight.state <= KnightStates::sword_cut_down) // cut state
        {
            ...
            if (delta > DELTA_BEGIN_ATTACK && delta < 5)
            {
                ...
                DoAttack(this, this.get_u16(s_str)*0.1f, attackAngle, attackarc, Hitters::sword, delta, knight);
            }
            else if (delta >= 8)
            {
                knight.swordTimer = 0;
        ...
        else if (knight.state == KnightStates::sword_power || knight.state == KnightStates::sword_power_super)
        {
            ...
            {
                DoAttack(this, this.get_u16(s_str)*0.2f, -(vec.Angle()), 120.0f, Hitters::sword, delta, knight );
            }
            else if (delta >= KnightVars::slash_time-0.5*getAgility(this))
            {
				knight.swordTimer = 0;
